...
typedef int (WINAPI *PFNISBATTLEMODE)();
PFNISBATTLEMODE p_IsBattleMode = NULL;
typedef int (WINAPI *PFNGETCURINCREMENTTURNINBATTLE)();
PFNGETCURINCREMENTTURNINBATTLE p_GetCurIncrementTurnInBattle = NULL;

typedef int (WINAPI *PFNGETREMAINTURNINBATTLE)();
PFNGETREMAINTURNINBATTLE p_GetRemainTurnInBattle = NULL;

typedef int (WINAPI *PFNGETCASTLETURNINBATTLE)();
...
namespace 関数 {
	...
	int Get_軍団の参戦立場(int 軍団番号【配列用】) {
		...
		if ( Is_野戦中() || Is_攻城戦中() ) {
			...
			//----------それ以外の場合。即ち、援軍系、中立軍系
			int attack_group_id = -1; // 攻撃側のグループID
			int defend_group_id = -1; // 守備側のグループID

			// 参加大名番号を検索して、攻撃側のグループID、守備側のグループIDを出す。
			for ( int ix=0; ix < GAMEDATASTRUCT_WAR_JOINT_DAIMYO_HEX_GROUP_INFO_NUM; ix++ ) {

				int iGundanNO = (int)nb6war_joint_daimyo_hex_group_info[ix].gundan;
				if ( 開始パラメタ.攻撃側軍団【軍団番号】 == iGundanNO ) {
					attack_group_id = nb6war_joint_daimyo_hex_group_info[ix].group;
				}
				else if ( 開始パラメタ.守備側軍団【軍団番号】 == iGundanNO ) {
					defend_group_id = nb6war_joint_daimyo_hex_group_info[ix].group;
				}
			}
			...
			for ( int ix=0; ix < GAMEDATASTRUCT_WAR_JOINT_DAIMYO_HEX_GROUP_INFO_NUM; ix++ ) {
				...
				if ( iGundanID == JoinGundanID ) {
					// 直接攻撃と同じグループならば…
					if ( nb6war_joint_daimyo_hex_group_info[ix].group == attack_group_id ) {
						return 参戦立場::攻撃の援軍; // 攻撃の援軍
					}
					// 直接防御と同じグループならば…
					else if ( nb6war_joint_daimyo_hex_group_info[ix].group == defend_group_id ) { 
...
namespace 関数 {
	...
	int Get_行動団体帰属(int 武将番号【配列用】) {
		for ( int i=0; i < GAMEDATASTRUCT_WAR_HEX_UNIT_INFO_NUM; i++ ) {
			if ( nb6bushou_hex_unit_info[i].bushou == 武将番号【配列用】+1 ) {
				return nb6bushou_hex_unit_info[i].junban;
			}
		}
		return 0;
	}
